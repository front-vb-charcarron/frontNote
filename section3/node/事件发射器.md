...
```
    ...
    channel.on('join', (id, client) => { // 加入聊天
        ...
        client.write('welcome!\nGuest online:' + channel.listeners('broadcast').length); // 返回在线人数
    });

    channel.on('leave', id => { // 离开聊天
        channel.removeListener('broadcast', channel.subScriptions[id]); //删除id指定的'broadcast'事件监听
        delete channel.subScriptions[id];
        channel.emit('broadcast', id, id + 'has left.');
    });

    channel.on('shutdown', () => { // 删除broadcast类型的所有监听事件，达到关闭聊天服务但服务器还运行
